  ...
  return (
    <div>
        ...
        <div className="flex items-center gap-3 shrink-0">
          {logoUrl ? (
            ...
            <div className="flex items-center gap-2">
              <LogoTori size={32} />
              <h1 className="text-2xl font-extrabold text-slate-900 dark:text-slate-100 tracking-tight whitespace-nowrap">
                Tori
              </h1>
            </div>
          )}
          {isTrial ? (
            <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 whitespace-nowrap">
            ...
            <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-slate-200 dark:bg-slate-700 text-slate-500 dark:text-slate-400 whitespace-nowrap">
              Básico
            </span>
          )}
        </div>
      ...
      {isTrial && diasRestantes !== null && diasRestantes <= 5 && (
        <div className="px-4 sm:px-6 pb-4">
          <div className="rounded-xl bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 px-4 py-3 flex items-center justify-between gap-3">
            ...
            <button
              onClick={onUpgrade}
              className="shrink-0 text-xs font-semibold text-amber-800 dark:text-amber-300 underline hover:no-underline"
            >
              Ver planes
            </button>
          </div>
      ...
      {!isTrial && plan === 'basic' && !diasRestantes && (
        <div className="px-4 sm:px-6 pb-4">
          <div className="rounded-xl bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-4 py-3 flex items-center justify-between gap-3">
            <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-400">
              Estás en el plan Básico. Actualiza a Pro para envíos ilimitados y más funciones.
            </p>
          <button
            onClick={onUpgrade}
            className="shrink-0 text-xs font-semibold text-sky-600 dark:text-sky-400 underline hover:no-underline"
          >
            Ver planes
          </button>
          </div>
